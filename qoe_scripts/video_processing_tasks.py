...
@ray.remote
def convert_to_yuv(ffmpeg_path, presenter, viewer, debug):
    ...
    if debug:
        ...
        out = sp.DEVNULL
    errout = sp.STDOUT
    ffmpeg_command_to_save_presenter = ' '.join([
        ffmpeg_path, "-n", "-threads", "1",
        "-i", presenter, "-c:v", "rawvideo", "-pix_fmt", "yuv420p", "-async", "1", "presenter.yuv", "-c:a", "pcm_s16le", "-async", "1", "presenter.wav",])
    process_presenter = sp.Popen(ffmpeg_command_to_save_presenter,
                             stdout=out, stderr=errout, shell=True)
    ffmpeg_command_to_save_viewer = ' '.join([
        ffmpeg_path, "-n", "-threads", "1",
        "-i", viewer, "-c:v", "rawvideo", "-pix_fmt", "yuv420p", "-async", "1", "viewer.yuv", "-c:a", "pcm_s16le", "-async", "1", "viewer.wav",])
    process_viewer = sp.Popen(ffmpeg_command_to_save_viewer,
                                stdout=out, stderr=errout, shell=True)
    process_presenter.wait()
    process_viewer.wait()
    return "presenter.yuv", "viewer.yuv", "presenter.wav", "viewer.wav"
